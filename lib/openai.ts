...
- "currency": ISO 4217 code (THB, USD, EUR, GBP, JPY, SGD, AUD, MYR, IDR, INR, etc). Infer from symbols (\u00a3=GBP, \u20ac=EUR, \u00a5=JPY, \u0e3f=THB, RM=MYR, Rp=IDR, S$=SGD, A$=AUD). Bare amounts with NO currency symbol → THB (never USD). USD only when a lone "$" is clearly printed (not S$/A$/C$/HK$). Myanmar or Thai script on the receipt → THB. Default to THB when nothing suggests another currency — common on Thai / SEA / Burmese POS receipts.
