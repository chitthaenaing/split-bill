...
- "printedItemUnits": when the receipt prints a footer like "Items: 7" / "Item(s): 7" / "Qty: 7" counting sold units, put that number here. It must equal sum(items[i].quantity). Use 0 when no such count is printed. If Items: N counts a bare numeric PLU/SKU row with no price (e.g. "1133371101"), omit that row from items and set printedItemUnits to the priced-unit sum — do NOT inflate other dishes' quantities to force a match.
