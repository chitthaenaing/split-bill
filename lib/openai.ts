...
- "items": every priced product / food / drink / merchandise / parts / repair labor / workshop fee AND free-item promotion lines that print as a minus amount tied to a product (e.g. "Promotion Free Tea -50.00") — those freebies MUST be their own item with a NEGATIVE price. Only combine sub-modifiers / toppings / notes into the parent line when they have NO amount in the price column. Skip headers, dividers, server names, table numbers, payment method lines, change lines, bank-transfer notes, crossed-out empty rows, and anything labelled subtotal / total / amount due / tax / VAT / tip / rounding. Bill-level Discount / Promotion Tier Discount / % off / Total Savings / voucher lines go in "discount" (positive amount) — NOT in items. On F&B receipts, restaurant "% service charge" / delivery / packaging / cover / bag / corkage go in tax, serviceCharge, rounding, or additionalCharges — never in items. On garage / repair invoices, parts AND labor (Thai ค่าแรง…), technician fees, and priced pickup/deposit lines stay in items — they are NOT restaurant service charge.
...
- "quantity": units of this item on this line, as printed in the qty / count column (often the leftmost number on the row). Do NOT default to 1 when a leading digit is printed. Do NOT confuse Table / Guests / Staff IDs above the items with line quantities. Put the digit in "quantity", not in "name" (name should be "Kya Saint", not "2 Kya Saint"). Only use 1 when the receipt truly shows one unit or omits qty. NEVER move a quantity digit from one product row onto a different product name (e.g. do not put Daily Special's "4" onto a Tea Leaf Salad row). Do NOT copy quantities from memory of similar dish names on other receipts (e.g. "Pone Mhan" / "Pone Hman" is sometimes 2 and sometimes 1 — read the digit on THIS row). On handwritten service forms without a qty column, use quantity=1 per priced row.
- "printedItemUnits": when the receipt prints a footer like "Items: 7" / "Item(s): 7" / "Qty: 7" counting sold units, put that number here. It must equal sum(quantity of items with price ≥ 0) — do NOT count minus promotion / discount rows toward this. Use 0 when no such count is printed. If Items: N counts a bare numeric PLU/SKU row with no price (e.g. "1133371101"), omit that row from items and set printedItemUnits to the priced-product-unit sum — do NOT inflate other dishes' quantities to force a match.
...
- "discount": bill-level Discount / Promotion Tier / % off / Total Savings / voucher amount as a POSITIVE number (฿27.90 off → 27.90). Do NOT put free-item promos here — those stay in items with a negative price. Use 0 when there is no bill-level discount.
...
- Handwritten / filled-in forms (common for Thai garage and motorcycle service): read blue/black ink carefully. Every filled amount-column row is an item unless it is clearly รวม / total. Do NOT treat crossed-out empty rows (Z / zig-zag through blank lines) as items. Ignore payment notes like "โอนธนาคาร …" / "โอนเงินแล้ว". Keep Thai names in "name"; put short English glosses in nameTranslated (e.g. drive belt, spark plug, service labor). Do NOT invent VAT or restaurant service % that are not written on the form.
...
- Before answering, run this self-check and fix anything that fails:
    ...
    sum(items[i].quantity where price \u2265 0) should equal printedItemUnits when printedItemUnits > 0 (e.g. "Items: 7") — minus promo lines do not count
  ...
  If sum(product quantity) is short of "Items: N", re-read the leftmost digit on every product row — do not leave quantity at 1 when 2+ is printed. Also look for a dropped multi-qty product line (Daily Special / set menus) whose units explain the shortfall — do not inflate a different row's quantity to fake the Items count. If the shortfall is only a bare unpriced PLU/SKU code row, omit that code and lower printedItemUnits instead. A correctly extracted minus promotion does not make Items: N "too low".
  ...
  If tax+service make the total too high by a free-item promotion amount, you missed a minus freebie line — add it to items. If the gap matches a printed Discount / Tier / % off line, put that amount in "discount" instead.
...
export const EXTRACTION_BILL_SCHEMA = {
  ...
  properties: {
    ...
    discount: {
      type: "number",
      description:
        "Bill-level discount / tier promo / % off as a positive amount. 0 when absent. Free-item promos stay in items as negative prices.",
    },
